...
No. The retrieval tool schema exposes only the query, and `_execute_tool` applies the application-selected access role rather than model-generated tool arguments. A regression test covers that boundary.
...
Yes if treated as the security boundary, which it is not. The string heuristic catches obvious requests but cannot prove prompt-injection safety. The important controls are deterministic tool allowlists, application-selected retrieval scope, restricted outbound API access, bounded memory and human approval for risky actions. A production evaluation suite should include adversarial prompt-injection cases.
...
- `orchestrator.py` — tool execution and retrieval scope
